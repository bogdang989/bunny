...
public class SBBindingHelper extends SBBeanHelper {
   ...
   */
  public static Object evaluateOutputEval(SBJob job, Object output, Object binding) throws SBExpressionException {
    Object outputEval = getOutputEval(binding);
    if (SBExpressionBeanHelper.isExpression(outputEval)) {
      try {
        return SBExpressionBeanHelper.evaluate(job, output, outputEval);
      } catch (SBExpressionException e) {
        throw new SBExpressionException("Failed to evaluate outputEval.", e);
      }
    }
    return output;
  }
