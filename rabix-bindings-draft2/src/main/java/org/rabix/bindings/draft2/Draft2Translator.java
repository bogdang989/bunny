...
public class Draft2Translator implements ProtocolTranslator {
  ...
  private DAGNode transformToGeneric(String globalJobId, Draft2Job job) throws BindingException {
    ...
    for (ApplicationPort port : job.getApp().getInputs()) {
      ...
      DAGLinkPort linkPort = new DAGLinkPort(Draft2SchemaHelper.normalizeId(port.getId()), job.getId(), LinkPortType.INPUT, LinkMerge.merge_nested, PickValue.only_non_null, port.getScatter() != null ? port.getScatter() : false, defaultValue, null);
      inputPorts.add(linkPort);
    }
    List<DAGLinkPort> outputPorts = new ArrayList<>();
    for (ApplicationPort port : job.getApp().getOutputs()) {
      DAGLinkPort linkPort = new DAGLinkPort(Draft2SchemaHelper.normalizeId(port.getId()), job.getId(), LinkPortType.OUTPUT, LinkMerge.merge_nested, PickValue.only_non_null, false, null, null);
